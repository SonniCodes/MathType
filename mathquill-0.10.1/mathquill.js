...
(function () {
    ...
    function getInterface(v) {
        ...
        var AbstractMathQuill = (APIClasses.AbstractMathQuill = P(Progenote, function (_) {
            ...
            _.__mathquillify = function (classNames) {
                ...
                root.jQ = $('<math-field class="mq-root-block"/>').attr(mqBlockId, root.id).appendTo(el);
    ...
    */
    ...
    var Variable = P(Symbol, function (_, super_) {
        _.init = function (ch, html) {
            super_.init.call(this, ch, '<var class="' + (html || ch) + '">' + (html || ch) + "</var>");
        };
        _.text = function () {
    ...
    var SummationNotation = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mq-large-operator mq-non-leaf">' + '<span class="mq-to"><span><small>&1</small></span></span>' + '<big class="sum">' + html + "</big>" + '<span class="mq-from"><span><small>&0</small></span></span>' + "</span>";
    ...
    var Matrix = (LatexCmds.matrix = P(MathCommand, function (_, super_) {
        ...
        _.createBlocks = function () {
            ...
            this.htmlTemplate.replace(/&\d+/g, function (match, index) {
                var row = cmd.htmlTemplate.substring(0, index).match(/<tr[^>]*>/gi).length - 1;
        ...
        _.parser = function () {
            var regex = Parser.regex,
                self = this,
                matrixName = this.getMatrixName(),
                rgxContents = new RegExp("^(.*)\\\\end{" + matrixName + "}"),
                rgxEnd = new RegExp("\\\\end{" + matrixName + "}");

            return regex(rgxContents).then(function (a) {
    ...
    var IntegralNotation = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mq-large-operator mq-non-leaf">' + html + '<span style="display: inline-flex; flex-direction: column; margin-left:-5px; margin-bottom: 3px; margin-right: -7px; position:absolute;">' + '<span style="display: inline-flex; margin-left:6px"><small>&1</small></span>' + '<span style="display: inline-flex; height: 1.8em;"></span>' + '<span style=" display: inline-flex; margin-left:-6px"><small>&0</small></span>' + "</span>" + "</span>";
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
        _.latex = function () {
            function simplify(latex) {
    ...
    var biggr = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = "<big>&0</big>";
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
        _.latex = function () {
            function simplify(latex) {
    ...
    var EvaluatedAt = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mq-non-leaf" style="0.2em; margin-right:0.2em;">' + '<span class="integral-fix"><span class="integral-fix"><small>&1</small></span></span>' + '<svg style="height:1em; width:auto;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 500"><rect x="6" y="0" width="15" height="500"/></svg>' + '<span class="integral-fix"><span class="integral-fix" style = ""><small>&0</small></span></span>' + "</span>";
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
        _.latex = function () {
            function simplify(latex) {
            ...
            return "\\rvert" + "_" + simplify(this.ends[L].latex()) + "^" + simplify(this.ends[R].latex());
        };
        _.parser = function () {
            var string = Parser.string;
            var optWhitespace = Parser.optWhitespace;
    ...
    var Limit = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mq-large-operator mq-non-leaf">' + '<span style = "margin-bottom: 0px; margin-right:0px !important; margin-bottom: -5px;">' + html + "</span>" + '<div class="mq-from" style = "margin-top: -6px !important;"><small>&0</small></div>' + "</span>";
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
    ...
    var Hat = (LatexCmds.hat = P(SquareRoot, function (_, super_) {
        _.htmlTemplate = '<span class="mq-non-leaf">' + '<span class="mq-vector-prefix"><svg viewBox="0 0.5 300 100" style="width="auto"; height="0.2em"; line-height: ;> <path style="stroke: rgba(0, 0, 0, 0);" d="M 0 135 L 145 0.6 L 300 127.516 L 145 30 L 0 135 Z"></path> </svg></span>' + '<span class="mq-vector-stem" style="margin-top:-2px">&0</span>' + "</span>";
        _.textTemplate = ["hat(", ")"];
        _.latex = function () {
            return "\\hat{" + this.ends[R].latex() + "}";
        };
    }));
    ...
    var Bracket = P(P(MathCommand, DelimsMixin), function (_, super_) {
        ...
        _.oppBrack = function (opts, node, expectedSide) {
            ...
            return node instanceof Bracket && node.side && node.side !== -expectedSide && (this.sides[this.side].ch === "|" || node.side === -this.side) && (!opts.restrictMismatchedBrackets || OPP_BRACKS[this.sides[this.side].ch] === node.sides[node.side].ch || { "(": "]", "[": ")" }[this.sides[L].ch] === node.sides[R].ch) && node;
        };
